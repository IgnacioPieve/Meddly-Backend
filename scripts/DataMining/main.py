...
def generate_merged_dataframe():
    ...
    for disease, count, symptom in rows:
        ...
        if disease is not None:
            ...
            for actual_disease in actual_diseases:
                file_1_diseases[actual_disease] = {
                    "count": count,
    ...
    for df_index in df.index:
        df.at[df_index, "disease"] = get_umls_codes(df.at[df_index, "disease"])[0]
        all_diseases_file2.add(df.at[df_index, "disease"])
...
def generate_indexes(all_symptoms, all_diseases):
    ...
    def generate_index(index_dataframe, index_name, must_be_in=None):
        ...
        schema = Schema(
            code=ID(stored=True),
            description=TEXT(analyzer=SimpleAnalyzer(), stored=True),
        )

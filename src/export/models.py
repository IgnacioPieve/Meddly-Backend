...
class UserDataPDFGenerator:
    ...
    def generate_base_file(data):
        template = template_env.get_template(
            "router/export/pdf_generator/assets/index.html"
        )
        if (
        ...
        return pdfkit.from_string(
            template_string,
            css="router/export/pdf_generator/assets/styles.css",
            options=options,
        )
    ...
    def generate_footer_list(pages):
        def get_image_logo():
            with open(
                "router/export/pdf_generator/assets/logo.png", "rb"
            ) as image_file:
        ...
        template = template_env.get_template(
            "router/export/pdf_generator/assets/footer.html"
        )
        template_string = template.render(**data)
    ...
    def get_user_data(user: User):
        ...
        for medicine_object in medicine_objects:
            medicine_object: user_calendar.models.medicine.Medicine
    ...
    @classmethod
    def generate_pdf(cls, user: User):
        base_file = cls.generate_base_file(cls.get_user_data(user))

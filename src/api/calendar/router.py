...
async def get_calendar(
    ...
    supervised_users_ids = list({supervised.id for supervised in await get_supervised(user)})
    users_to_get_calendar = users or (supervised_users_ids + [user.id])

    if any(user_to_get_calendar not in supervised_users_ids
           and user_to_get_calendar != user.id
           for user_to_get_calendar in users_to_get_calendar):
        raise ERROR205

    user_objects = await asyncio.gather(*[get_user(user_to_get_calendar) for user_to_get_calendar in users_to_get_calendar])
    calendar = {user.id: await get_calendar_service(user, start, end) for user in user_objects}
